...
const questions = require('./newquestions');
...
var client = new okta.Client({
  orgUrl: 'https://dev-124700.oktapreview.com',
  token: 'Your Token!',    // Obtained from Developer Dashboard
...
function populateRoundAnswers(
  ...
  //console.log('translatedQuestion 1', translatedQuestion);
  //const translatedQuestion = questions.QUESTIONS_EN_US[correctAnswerIndex].question;
  //const answersCopy = translatedQuestion[Object.keys(translatedQuestion)[0]].slice();
  const answersCopy = translatedQuestion.answers;
  
   
//  console.log('answers 2', answersCopy2);
...
function handleUserGuess(userGaveUp, handlerInput) {
  ...
  if (answerSlotValid
    ...
    console.log('translated questions', translatedQuestions[gameQuestions]);
    if(translatedQuestions[gameQuestions[currentQuestionIndex]].genre == 'Hip Hop'){
        client.getUser('Your ID')
       .then(user => {
         user.profile.hiphopScore++;
         user.update();
      });
    }else if(translatedQuestions[gameQuestions[currentQuestionIndex]].genre == 'pop'){
        client.getUser('Your ID')
       .then(user => {
         user.profile.popScore++;
         user.update();
      });
    }else if(translatedQuestions[gameQuestions[currentQuestionIndex]].genre == 'jazz'){
        client.getUser('Your ID')
       .then(user => {
         user.profile.jazzScore++;
         user.update();
      });
    }
  ...
  Object.assign(sessionAttributes, {
    ...
    score: currentScore,
    correctAnswerText: translatedQuestion.answers[0]
  });

  console.log('trnaslated question 2', translatedQuestion);
  console.log('correct answer text 2 ', correctAnswerText);
...
function startGame(newGame, handlerInput) {
  ...
  //console.log('trnaslated question 1', translatedQuestion);
  //console.log('correct answer text 1', correctAnswerText);
...
const statshandler = {
  ...
  handle(handlerInput){
    ...
    return new Promise((resolve, reject) => {
      client.getUser('Your ID')
      .then(user => {
        js = user.profile.jazzScore;
        ps = user.profile.popScore;
        hs = user.profile.hiphopScore;
          resolve(handlerInput.responseBuilder.speak(`Your jazz score is ${js}, your pop score is ${ps}, your hip hop score is ${hs}`).getResponse()) ;
        }).catch((error) => {
          return handlerInput.responseBuilder
    ...
    //return handlerInput.responseBuilder.speak('foo').getResponse();
  }
};
